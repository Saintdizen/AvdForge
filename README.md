...
CI (`.gitverse/workflows/build_linux.yml`) собирает AppImage на push/PR в `master`, а релиз
публикует только по тегу `v<версия>`, где `<версия>` совпадает с `version` в `package.json`.
Секрет `AUTH` — публичный API-ключ GitVerse с правом «Репозитории → Запись».
...
* CI собирает только Linux-сборку (AppImage): Windows-сборка (`npm run win`) идёт с `--publish always`
  и требует настроенного паблишера, которого для GitVerse нет.
